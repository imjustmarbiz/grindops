...
export const queueConfig = pgTable("queue_config", {
  ...
  bidWarNotificationsEnabled: boolean("bid_war_notifications_enabled").notNull().default(true),
  embedThumbnailUrl: text("embed_thumbnail_url"),
  earlyAccessMode: boolean("early_access_mode").notNull().default(false),
  allowedAccessRoles: jsonb("allowed_access_roles").$type<string[]>().default(["owner", "staff", "grinder", "elite", "creator"]),
