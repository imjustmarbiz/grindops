    ...
    <div className="fixed bottom-0 left-0 right-0 z-[60] pointer-events-auto" data-testid="site-alert-ticker">
      <div className="relative border-t border-primary/30 backdrop-blur-md shadow-[0_-4px_20px_rgba(0,0,0,0.4)] bg-[hsl(var(--background)/0.95)]">
        ...
        <div className="flex items-center h-9 sm:h-10">
          <div className="shrink-0 flex items-center gap-1.5 px-3 sm:px-4 border-r border-primary/20 h-full bg-primary/10">
            <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-primary">Alert</span>
          </div>

          <div className="flex-1 min-w-0 px-3 sm:px-4 flex items-center justify-center">
            <p className="text-xs sm:text-sm font-bold text-foreground tracking-wide truncate text-center" data-testid="text-alert-message">
              {combinedMessage}
            </p>
          </div>
          ...
          <button
            onClick={dismissAll}
            className="shrink-0 px-2 sm:px-3 h-full flex items-center hover:bg-primary/10 transition-colors border-l border-primary/20"
