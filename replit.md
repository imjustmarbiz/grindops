...
- **Badge Progression System:** 50 total badges (35 auto-earned, 15 staff-awarded) across 3 tiers (short/mid/long-term) spanning 0-5+ years. Orders: 1/5/10 (short) → 25/50 (mid) → 100/250/500 (long). Earnings: $500/$2K (short) → $5K/$10K (mid) → $25K/$50K/$100K (long). Loyalty: 7d/90d (short) → 6mo/1yr (mid) → 2yr/5yr (long). Auto badges include: versatile (3+ roles), top-tier (Diamond/Elite tier). Manual badges include: problem-solver, above-beyond, comeback-king. Xbox badge = green "A" button, PS5 badge = blue "X" cross button. BADGE_META includes `tier` field. TIER_LABELS/TIER_COLORS exported for UI display.
...
- **Fine Payment System:** Grinders can submit fine payments with screenshot proof and payment method (Zelle, PayPal, Apple Pay, Cash App, Venmo). Staff reviews submissions with approve/deny workflow. Approved fines are added to company profit via `fineRevenue` in analytics. Schema: `fine_payments` table with `grinderId`, `amount`, `paymentMethod`, `proofUrl`, `status` (pending/approved/denied), review fields. Routes: `POST /api/grinder/me/fine-payments` (file upload), `GET /api/grinder/me/fine-payments`, `GET /api/staff/fine-payments` (requireStaff), `PATCH /api/staff/fine-payments/:id/review` (requireStaff). Fine revenue included in `AnalyticsSummary.fineRevenue` and added to `totalCompanyProfit` and `totalRevenue`. Upload dir: `uploads/fine-proofs/`.
- **Scorecard Strike/Fine History:** Both staff and grinder scorecard views now include a "Strike & Fine History" section showing all strike logs with action type (+/-Strike badge), resulting strike count, reason, fine amount, paid/unpaid status, and timestamps. Staff scorecard API (`GET /api/staff/grinder-scorecard/:grinderId`) and grinder scorecard API (`GET /api/grinder/me/scorecard`) both return `strikeLogs`.
- **Need Replacement → Emergency Orders:** When an order is set to "Need Replacement" status, the status is preserved as "Need Replacement" (not converted to "Open"). The order gets `isEmergency = true` and appears in grinders' emergency/replacement available orders section. Grinder available orders filter includes "Need Replacement" status alongside "Open" and "Bidding Closed".
