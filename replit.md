...
- **Holiday Theme System:** Owner-only toggle in Admin → System tab to apply seasonal dark theme variations for US holidays. Supported themes: Christmas, Thanksgiving, 4th of July, Halloween, Valentine's Day, New Year's Eve, St. Patrick's Day. Each theme modifies background hues, accent colors, and scrollbar colors via CSS custom properties. Stored in `queue_config.holiday_theme` column. API: `PATCH /api/config/holiday-theme`. Applied globally via `layout.tsx` adding holiday CSS class to `document.documentElement`.
