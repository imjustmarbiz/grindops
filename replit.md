...
- **Enhanced Grinder Scorecard:** The "My Scorecard" page now includes Tier Progress visualization, Payout Summary (total paid, pending, recent/pending payouts list), Completed Orders history, and a permanently visible Strike & Fine History section (shows "Clean record" when empty). Synthetic order logs use `grinderEarnings` for accurate pay display.
