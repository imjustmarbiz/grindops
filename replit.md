...
- **Dashboard KPI Cards:** Compact StatCard components with click-to-navigate behavior and link labels. Owner cards link to Business, Payouts, and Orders pages. Staff cards link to Orders, Bids, Grinders, and Payouts pages. Cards have keyboard accessibility (role="button", tabIndex, Enter/Space handlers) and data-testid attributes.
- **Financial Management:** Includes a Payout Reduction System, Deletion Request Workflow, Fine Payment System, and a comprehensive Business Wallet System with detailed financial tracking, transfer capabilities, and robust notification systems. Dedicated Creator Payouts page at `/creator-payouts` (staffOnly) mirrors grinder payouts flow — pending/approved/paid/rejected lifecycle, wallet deduction (owner-only), payment proof upload, filtering by status/creator/date. API: `GET/PATCH /api/staff/creator-payouts` endpoints. Creator payout requests (from `business_payouts` table with `recipientRole="Creator"`) now appear as action items on the To-Do List and trigger owner notifications.
- **Admin Functionality:** Combined admin page for operations, management, and system settings, including toggles for MGT Bot data tracking, maintenance mode, order ID editing, early access mode, and holiday theme selector.
- **Holiday Theme System:** Owner-only toggle in Admin → System tab. 7 seasonal dark themes (Christmas, Thanksgiving, 4th of July, Halloween, Valentine's Day, New Year's Eve, St. Patrick's Day) with both CSS color overrides and animated decorative elements (snowflakes, spider webs, bats, falling leaves, hearts, confetti, clovers, twinkling lights, fireworks). Stored in `queue_config.holiday_theme`. API: `PATCH /api/config/holiday-theme`.
- **Game Theme System:** Owner-only toggle in Admin → System tab. Supports game-specific UI themes starting with NBA 2K26 — warm orange/brown tones, faint basketball court line overlay, large "2K" watermark, and floating basketball particles. Stored in `queue_config.game_theme`. API: `PATCH /api/config/game-theme`. Both theme systems use `ThemeDecorations` component (`client/src/components/theme-decorations.tsx`) for animated overlays, applied globally via `layout.tsx`.
- **Site Alerts System:** Owners can broadcast scrolling ticker-bar alerts with targeting options for various user groups.
- **Creator Role System:** Creators are assigned the `creator` role via admin panel or impersonation. The role is preserved during Discord role-refresh cycles (`/api/auth/user`) and OAuth login callbacks — if a user has `role=creator` in the DB and Discord determines `none`, the creator role is kept. Creator routes use `isAuthenticated` middleware (not `requireGrinderOrStaff`). Edit/delete controls for creators in Admin panel are gated to owners only.
...
- **User Activity Log:** Activity log page removed from sidebar navigation (page file retained for reference).
- **Staff Auto To-Do List:** System-generated action items based on order and assignment statuses, grouped by category and sortable by priority, with staff dismissal functionality. Sidebar shows amber pulsing badge with pending count (action items + pending tasks) for both staff and grinder dashboards.
- **Sidebar Badges:** Notifications = blue pulsing badge (unread count). To-Do List = amber pulsing badge (pending action items + tasks). "Tier Progress" renamed to "Grinders Performance" in sidebar and page title.
